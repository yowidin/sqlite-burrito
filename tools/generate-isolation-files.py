...
def write_isolation_header(args, expressions):
    ...
    print('// almost like some sort of... module... 🤔?', file=f)
    print('', file=f)
...
def write_isolation_source(args, strings):
    f = StringIO()
    ...
    print('#include <sqlite3.h>', file=f)
    print('', file=f)

    print('using namespace sqlite_burrito;', file=f)
    print('', file=f)

    print('static_assert(isolation::sqlite_version_number >= SQLITE_VERSION_NUMBER, '
          '"SQLite3 versions should match");', file=f)
    print('', file=f)
...
def main():
    ...
    while i < len(lines):
        ...
        def handle_char_def():
            ...
            if m:
                ...
                expressions.append('   static const char * const %s;' % name)
                string_defines[name] = value
        ...
        def handle_int_def():
            ...
            if m:
                ...
                expressions.append('   static constexpr int %s = %s;' % (name, value))
                int_defines[name] = value
                return True
    ...
    write_isolation_source(args=args, strings=string_defines)
